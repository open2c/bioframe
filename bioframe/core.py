...
def _verify_columns(df, colnames, return_as_bool=False):
    ...
    if not type(df) is pd.core.frame.DataFrame:
        if return_as_bool: return False
...
def _verify_column_dtypes(df, cols=None, return_as_bool=False):
    ...
    if not _verify_columns(df, 
        [ck1, sk1, ek1], return_as_bool=True):
        if return_as_bool: return False
...
def is_contained(
    ...
    if not is_cataloged(df, view_df, df_view_col="view_region"):
        return False
...
def is_covering(
    df, view_df, view_name_col="name", cols=None
):
    """
...
def is_tiling(df, view_df, df_view_col="view_region", view_name_col="name", cols=None):
    ...
    if not is_covering(df, view_df, view_name_col=view_name_col, cols=None):
        print('not covered')
        return False
    if not is_contained(df, view_df, df_view_col=df_view_col, view_name_col=view_name_col, cols=None):
        print('not contained')
        return False
    return True
...
def sanitize_bedframe(
    df1, drop_null=True, return_sorted=True, drop_invalid=True, flip_invalid=True, cols=None
):
    """
    Attempts to clean a genomic interval dataframe to be valid.

    <<<TODO: discuss if sorting should should accept a tuple of column names instead>>>
    <<< also needs a view_df! >>>
    return_sorted:bool
    ...
    if drop_invalid:
        out_df = out_df.iloc[((out_df[ek1] - out_df[sk1]) >= 0).values]

    if ((out_df[ek1] - out_df[sk1]) < 0).any() and flip_invalid:
...
def is_viewframe(region_df, raise_errors=False, view_name_col="name", cols=None):
    ...
    if not _verify_columns(region_df, [ck1, sk1, ek1, view_name_col], return_as_bool=True):
        if raise_errors:
            raise TypeError("Invalid columns")
        return False
    ...
    if len(set(region_df[view_name_col])) < len(region_df[view_name_col].values):
        if raise_errors:
            raise ValueError(
                "Invalid region dataframe: entries in region_df[view_name_col] must be unique"
            )
        return False
    ...
    if is_overlapping(region_df, cols=cols):
        if raise_errors:
            raise ValueError(
                "Invalid region dataframe: entries must be non-overlapping"
            )
        return False
...
def make_viewframe(
    ...
    if is_viewframe(view_df, view_name_col=view_name_col, cols=(ck1, sk1, ek1)):
        return view_df
    else:
