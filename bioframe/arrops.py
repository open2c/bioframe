...
def overlap_intervals(starts1, ends1, starts2, ends2):
    ...
    overlap_ids : numpy.ndarray
        ...
        The 1st column contains ids from the 1st set, the 2nd column has ids from the 2nd set.
...
def _closest_intervals_nooverlap(
    ...
    upstream_ids, downstream_ids = np.zeros((0,2), dtype=int), np.zeros((0,2), dtype=int)
    ...
    if k_upstream > 0:
        ...
        int1_ids = np.repeat(np.arange(n1), upstream_closest_endidx - upstream_closest_startidx)
        int2_sorted_ids = arange_multi(upstream_closest_startidx, upstream_closest_endidx)
...
#             arange_multi(upstream_closest_startidx - upstream_closest_endidx, 0)
    ...
    if k_downstream > 0:
        ...
        downstream_closest_endidx = np.minimum(downstream_closest_startidx + k_downstream, n2)

        int1_ids = np.repeat(
            np.arange(n1), 
            downstream_closest_endidx - downstream_closest_startidx)
        int2_sorted_ids = arange_multi(downstream_closest_startidx, downstream_closest_endidx)
